# 猫爪形状测试点 (Neko Paw Shaped Test Point)

中文说明 | [English](README.md)

本仓库包含了“猫爪”形状的测试点设计文件，包括 KiCad 封装和 FreeCAD 测试点夹具设计文件。
...
所有封装的 Edge Cuts 层都包含了定位孔。没有定位孔的封装即将推出。

## 测试点夹具 (Connector)
当然，本仓库也包含了猫爪测试点的夹具。它们都需要探针 (pogo pin) 来进行组装。

我们强烈建议使用针管直径为 1.0mm、针头直径为 0.74mm 的圆头探针，例如 PAL75-J。如果你使用尖头探针，它可能会损坏你 PCB 的焊盘。

你还需要两个直径为 1.0mm、长度为 16mm 的定位销 (dowel pin)。

有两种类型的夹具。

### 测试探针夹具 (Test Needle Connector)
测试探针夹具是一种手持式的测试工具。这种夹具非常适合用于批量烧录芯片。
...
### 夹子夹具 (Clip connector)
夹子夹具是一种稳定连接到 PCB 的好方法，非常适合用于芯片调试。`Clip` 文件夹包含了一个 FreeCAD 设计文件 `Fixture.FCStd`，你可以使用 PETG 或其他坚固的 3D 打印材料来打印它。配套的转接 PCB 设计也包含在 `Clip` 文件夹中。

你需要使用三个 M2 * 12mm 螺丝来组装这个夹具，其中两个用于固定夹子主体和转接 PCB，一个用于组装夹子部件。

这种类型的夹具需要一个线径 0.7mm * 外径 7mm * 长度 20mm 的弹簧来配合使用。
